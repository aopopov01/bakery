// Simple Database Configuration using better-sqlite3
...
class SimpleDatabaseManager {
  ...
  async createTables() {
    ...
    try {
      this.runQuery('CREATE INDEX IF NOT EXISTS idx_users_email ON users(email)');
      this.runQuery('CREATE INDEX IF NOT EXISTS idx_orders_user_id ON orders(user_id)');
      this.runQuery('CREATE INDEX IF NOT EXISTS idx_orders_status ON orders(status)');
      this.runQuery('CREATE INDEX IF NOT EXISTS idx_security_logs_user_id ON security_logs(user_id)');
      this.runQuery('CREATE INDEX IF NOT EXISTS idx_security_logs_created_at ON security_logs(created_at)');
    } catch (error) {
  ...
  async createDefaultAdmin() {
    try {
      const existingAdmin = this.getQuery(
    ...
    for (const product of products) {
      try {
        ...
        const existing = this.getQuery(
          'SELECT id FROM products WHERE name_bg = ?',
          [product.name_bg]
        );
